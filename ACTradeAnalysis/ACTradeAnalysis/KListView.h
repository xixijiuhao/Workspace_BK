...
{
	typedef std::function<void(string)> OnClickListItem;
	typedef std::map<int, std::vector<string>> ListTotalData;		//整体数据
	typedef std::map<std::string, wstring> MapExchangeData;		    //交易所中英文
	typedef ListTotalData::iterator listIterator;						
	typedef std::vector<KListViewHeaderData> headerData;				//Header数据
	typedef std::vector<string> itemData;							//Item数据
...
private:
	...
	MapExchangeData		m_ExchangeData;
