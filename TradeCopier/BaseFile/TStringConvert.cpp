...
void        GetDisconnectReason(TSocketDisconnectReason reason, char* szText, int length)
{
	...
	{
	case sdrInitiative:
		strcpy_s(szText, length, "主动断开");
		break;
	case sdrPassive:
		strcpy_s(szText, length, "被动断开");
		break;
	case sdrReadError:
		strcpy_s(szText, length, "读错误，断开");
		break;
	case sdrWriteError:
		strcpy_s(szText, length, "写错误，断开");
		break;
	case sdrBufFill:
		strcpy_s(szText, length, "缓冲区满，断开");
		break;
	case sdrAsynError:
		strcpy_s(szText, length, "异步操作错误，断开");
		break;
	case sdrParseError:
		strcpy_s(szText, length, "解析数据错误，断开");
		break;
	case sdrConnectOuttime:
		strcpy_s(szText, length, "连接超时，断开");
		break;
	default:
