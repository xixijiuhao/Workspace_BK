...
void CMouseComboBox::Init(HWND hComboBox)
{
	CxSubClassWnd::Init(hComboBox);
	COMBOBOXINFO info;
	info.cbSize = sizeof(COMBOBOXINFO);
	::GetComboBoxInfo(m_hWnd,&info);
	m_Edit.InitEdit(info.hwndItem);
}
LRESULT CALLBACK CMouseComboBox::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	if (message == WM_PAINT)
	{
		COMBOBOXINFO Info;
		...
		InvalidateRect(m_Edit.GetHwnd(), NULL, TRUE);

		return 1;
	}
	return CallWindowProc((WNDPROC)m_OldProc, m_hWnd, message, wParam, lParam); //默认回调过程
}
void CMouseComboBox::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	TEXTMETRIC tm;
	HDC dc = GetDC(m_hWnd);
...
void CMouseComboBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
void CMouseComboBox::ResetContent()
{
...
void CMouseComboBox::GetLBText(int nIndex, wstring& rString) const
{
...
void CMouseComboBox::ResetSelection()
{
	ComboBox_SetCurSel(m_hWnd, -1);
	m_iSelectedRow = -1;
}
bool CMouseComboBox::LineInStringLeft(const string& rsLine, const string& rsString) const
{
...
bool CMouseComboBox::LineInStringRight(const string& rsLine, const string& rsString) const
{
	if (rsLine.length() > rsString.length())
		return false;
	string sPart;
...
int  CMouseComboBox::FindUniqueMatchedItem(const string &line, string &sMatchedLine)
{
	sMatchedLine.clear();
...
void CMouseComboBox::OnCbnEditupdate()
{
	
	string sMatchedText;
	string line = GetWindowTextAnsic(m_hWnd);
	...
	if (m_iSelectedRow >= 0)
	{
		...
		if (GetReverFind())
			SetWindowTextAnsic(m_hWnd, line.c_str());
