...
void CxCombox::AddString(const char* pszText, int index)
{
	SendMessage(m_hWnd,CB_INSERTSTRING,(WPARAM)index,(LPARAM)LoadRC::ansi_to_unicode(pszText).c_str());
}
...
const char* CxCombox::GetText()
{
	GetWindowTextA(m_hWnd, m_szContent, sizeof(m_szContent));
	return m_szContent;
}