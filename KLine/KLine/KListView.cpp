...
KListView::KListView() :
	...
	m_iItemHeight(30)
{
	m_textColor = RGB(217, 216, 228);
	m_bkColor = RGB(39, 38, 46);
	m_frameColor = RGB(71, 69, 76);
	m_headerColor = RGB(133, 129, 146);
	m_scrollColor = RGB(163, 161, 179);
	m_selectColor = RGB(57, 55, 64);
	//m_selectColor = RGB(201, 51, 39);
}
KListView::~KListView()
{
...
void KListView::OnPaint()
{
	...
	for (int index = 0 ; index < m_vtItemRect.size(); index++)
	{
		...
		DrawText(dc.GetHdc(), m_vtShowItemData[index].contractShowName, wcslen(m_vtShowItemData[index].contractShowName), &m_vtItemRect[index], DT_CENTER | DT_VCENTER | DT_SINGLELINE);
