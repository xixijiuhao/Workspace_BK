...
{
	style |= WS_CHILD | WS_VISIBLE | LVS_NOSORTHEADER | LVS_SINGLESEL | LVS_SHOWSELALWAYS|WS_CLIPSIBLINGS;
...
void TListView::AddItem(TCHAR *pszText, int iRow, int iCol, UINT text_format /* = 0 */, COLORREF textColor /* = -1 */)
{
	AddItem(pszText, iRow, iCol, text_format, textColor);
}
...
void TListView::AddItem(const char *pszText, int iRow, int iCol, UINT textFormat, COLORREF textColor)
{
	...
	if (iCol == 0)
	{
		/*int i = ListView_InsertItem(m_hwndList, &item);
		int j = GetLastError();
		i += j;*/
...
LRESULT TListView::NewHeaderProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR id, DWORD_PTR data)
{
	...
	case WM_PAINT:
		...
		return 0;
	case WM_LBUTTONDOWN:
		p->OnButtonDown(wParam, lParam);
		return 0;
		/*case WM_NOTIFY:
			PostMessage(::GetParent(m_hwndList), WM_LISTMESSGAE, wParam, lParam);
			p->OnNotify(wParam, lParam);
			return 0;*/
	case WM_ERASEBKGND:
		return 1;
	}
...
void TListView::OnButtonDown(WPARAM wParam, LPARAM lParam)
{

}

void TListView::OnNotify(WPARAM wParam, LPARAM lParam)
{
	int selected = ((LPLVCOLUMN)lParam)->iSubItem + 1;
	TCHAR szBuffer[10];
	LPCTSTR str1 = TEXT("第%i列被双击了");
	wsprintf(szBuffer, str1, selected);
	if (selected != 0)
		MessageBox(0, szBuffer, L"Test", MB_OK | MB_ICONASTERISK);
}
...
void TListView::AdjustScroll()
{
    ListView_Scroll(m_hwndList, 0, m_iVShift*m_iItemHeight);
	ListView_SetItemState(m_hwndList, m_iPreSelectIndex, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
}

void TListView::DeleteAllItems()
{
    m_iVShift = GetScrollPos(m_hwndList, SB_VERT);
	m_iPreSelectIndex = GetSelectItem();
