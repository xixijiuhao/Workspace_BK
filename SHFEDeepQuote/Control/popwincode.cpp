...
bool CPopWinCode::UpdateData(char * cSign)
{
	...
	vSortedExchange.push_back("SHFE");
	vSortedExchange.push_back("INE");

    for (size_t i = 0; i < vSortedExchange.size(); i++)//遍历交易所 交易所数量等于1自动展开
    {
        const char * strExchange = vSortedExchange.at(i);

		//if (!(strcmp(strExchange, "SHFE") == 0 || strcmp(strExchange, "INE") == 0)) continue;

		CMBLSpiHandler *hd = CMBLSpiHandler::GetMBLQuoteData();
		if(hd->QuoteTypes == "SHFE")
		{
			if (strcmp(strExchange, "SHFE")) 
				continue;
		}
		else if (hd->QuoteTypes == "INE")
		{
			if (strcmp(strExchange, "INE"))
				continue;
		}
        ...
        if (g_pStarApi->GetExchangeData(strExchange, &pExchange, 1, false) && pExchange)
        {
			...
			//屏蔽从交易模块获取合约列表，改从行情模块获取
            //g_pTradeData->GetCommoditys(commodityData, &exKey);
			SCommodity  * commodityData[50] = {};
			SRetType count = g_pStarApi->GetCommodityData(exKey.ExchangeNo,"", commodityData, sizeof(commodityData)/sizeof(commodityData[0]), true);
            for (size_t j = 0; j < count; j++)
            {
                SCommodity *pCommodity = commodityData[j];
				TCHAR log[100] = {};
				wsprintf(log, L"Commodity Type:%c\n", pCommodity->CommodityType);
				OutputDebugString(log);

                if (!pCommodity ||
					pCommodity->CommodityType != S_COMMODITYTYPE_FUTURES)
                {
                    break;
                }
	...
	//		{
	//			wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResWchar_t(IDS_Spread));
	//			wchar_t *q = wcsstr(pCommodity->CommodityName, LoadResWchar_t(IDS_SpreadM));
	//			if (!p&&!q)
	//				swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResWchar_t(IDS_Spread));
	//			else
	...
	//		{
	//			wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResWchar_t(IDS_Option));
	//			if (!p)
	//				swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResWchar_t(IDS_Option));
	//			else
	//				swprintf_s(strCommodity, L"%s[%s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);					
	//		}
	...
	//			{
	//				wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResWchar_t(IDS_Straddle));
	//				if (!p)
	//					swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResWchar_t(IDS_Straddle));
	//				else
	//					swprintf_s(strCommodity, L"%s[%s] ", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);
	//			}
	//			else if (pCommodity->CommodityType == ctSTG)
	//			{
	//				wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResWchar_t(IDS_Strangle));
	//				if (!p)
	//					swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResWchar_t(IDS_Strangle));
	//				else
	//					swprintf_s(strCommodity, L"%s[%s]  ", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);
	//			}
	//		}
	...
	case WM_CREATE:
		{
			...
			m_hListView = CreateWindowEx(0, WC_LISTVIEW,
				...
				m_nWidth *4/ 7, 0, m_nWidth *3/ 7, m_nHeight,
