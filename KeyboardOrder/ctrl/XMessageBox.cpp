...
protected:
	...
	enum
	{
		...
		ButtonHeight            = 14,		// dialog units  +++1.7
		ButtonSpacing           = 9,		//按钮间距
		BottomMargin            = 12,		//下边距
		SpacingBetweenMessageAndButtons = 4,
...
{
	IDS_ABORT,            _T("&Abort"),					//中断&A	
	IDS_CANCEL,           _T("Cancel"),					//取消
	IDS_CONTINUE,         _T("&Continue"),				//继续&C
	IDS_DONOTASKAGAIN,    _T("Don't ask me again"),		//不再询问此问题
	IDS_DONOTTELLAGAIN,   _T("Don't tell me again"),	//不再告知此信息
	IDS_DONOTSHOWAGAIN,   _T("Don't show again"),		//不再显示此信息
	IDS_HELP,             _T("&Help"),					//帮助&H
	IDS_IGNORE,           _T("&Ignore"),				//忽略&I
	IDS_IGNOREALL,        _T("I&gnore All"),			//忽略全部&G
	IDS_NO,               _T("&No"),					//否&N
	IDS_NOTOALL,          _T("N&o to All"),				//全否&O
	IDS_OK,               _T("&Ok"),					//确定
	IDS_REPORT,           _T("Re&port"),				//报告&P
	IDS_RETRY,            _T("&Retry"),					//重试&R
	IDS_SKIP,             _T("&Skip"),					//跳过&S
	IDS_SKIPALL,          _T("S&kip All"),				//全部跳过&K
	IDS_TRYAGAIN,         _T("&Try Again"),				//再试&T
	IDS_YES,              _T("&Yes"),					//是&Y
	IDS_YESTOALL,         _T("Y&es to All")				//全是&E
};
				...
				XMSGBOXPARAMS * pXMB /*= NULL*/)
{
		...
		(nStyle & MB_DONOTSHOWAGAIN))
	{
		...
		if (xmb.lpszModule && (xmb.lpszModule[0] != _T('\0')))
		{
			...
			{
				//saved DoNotAsk found, returning  dwData
				return (int)dwData;
			}
		}
	}
...
{
	//in CXDialogTemplate::CXDialogTemplate
	...
	//if (lpszCheckText)
	//	_tcscpy(g_ButtonText[eDoNotShowAgain].pszDefaultText, lpszCheckText);
	...
	if (lpszCheckText)
		_tcscpy(m_szButtonText[eDoNotShowAgain], lpszCheckText);
	///////////////////////////////////////////////////////////////////////////
	// get dc for drawing
	...
	{
		//ERROR:  Help button cannot be default button;
		_ASSERTE(FALSE);
	}

	if (nReportButton == m_nDefButton)
	{
		//ERROR:  Report button cannot be default button"));
		_ASSERTE(FALSE);
	}
	...
	if (m_bRightJustifyButtons)
		x = mbrect.right - nTotalButtonWidth - SpacingSize;		//+++1.7
	...
	m_dlgTempl.x = 0;
	m_dlgTempl.y = 0;
	m_dlgTempl.cx = (short)(MulDiv(mbrect.Width(), 4, nBaseunitX));			//+++1.8
	m_dlgTempl.cy = (short)(MulDiv(mbrect.Height(), 8, nBaseunitY));
								   ...
								   LPCTSTR lpszButtonCaption)
{
	...
	y += (m_nDoNotAskAgainHeight+  ButtonSpacing);

	buttonrow.bottom = y + m_nButtonHeight;
	mbrect.bottom = buttonrow.bottom + 2 * SpacingSize;
	if (mbrect.Width() < (x + w))
...
void CXDialogTemplate::LoadButtonStringsFromResources(HINSTANCE hInstance)
{
	...
	for (int index = 0; index < LAST_BUTTON; index++)
	{
		...
		if (index == eReport)
		{
			...
			if (m_szButtonText[index][0] == _T('\0'))
			{
				wstring w = LoadResString(g_ButtonText[index].resId);
				_tcscpy(m_szButtonText[index], w.c_str());
				if (w.empty())
					rc = 0;
				else
					rc = 1;
	...
	switch (message)
	{
		...
		case WM_INITDIALOG:
		{
			...
			else if (GetParent(hwnd) != NULL)
			{
				...
				::GetWindowRect(hwnd, &dlgRect);
				int x = mainRect.left	+ (int)((mainRect.Width()-dlgRect.Width())/2);
				int y = mainRect.top	+ (int)((mainRect.Height()-dlgRect.Height())*(1-0.618));
...
int MsgBox(HINSTANCE hInstance,HWND hwnd, LPCTSTR sText, LPCTSTR sTitle, UINT nStyle, COLORREF bkColor, COLORREF txtColor, XMSGBOXPARAMS::CUserDefinedButtonCaptions * btnCaptions)
{
	XMSGBOXPARAMS xmb;
	xmb.hInstanceStrings = hInstance;
	xmb.crBackground = bkColor;
	xmb.crText = txtColor;
	if (btnCaptions)
	{
		xmb.bUseUserDefinedButtonCaptions = TRUE;
		xmb.UserDefinedButtonCaptions = *btnCaptions;
	}

	return XMessageBox(hwnd, sText, sTitle, nStyle, NULL, &xmb);
}
//lmx end
