...
{
	PAINTSTRUCT ps;
	HDC hdctmp = BeginPaint(m_hwndHeader, &ps);

	RECT CRect;
	GetClientRect(m_hwndHeader, &CRect);
	HDC memdc = CreateCompatibleDC(hdctmp);
	HBITMAP bitmap = CreateCompatibleBitmap(hdctmp, CRect.right - CRect.left, CRect.bottom - CRect.top);
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (i > 0)
		{
			MoveToEx(memdc, rect.left, rect.top, 0);
			LineTo(memdc, rect.left, rect.bottom);
		}
		DrawText(memdc, col.pszText, -1, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	}

	BitBlt(hdctmp, 0, 0, CRect.right - CRect.left, CRect.bottom - CRect.top, memdc, 0, 0, SRCCOPY);

	DeleteObject(hbrBorder);
	DeleteObject(hPen);
	DeleteObject(bitmap);
	DeleteDC(memdc);
	EndPaint(m_hwndList, &ps);
}
