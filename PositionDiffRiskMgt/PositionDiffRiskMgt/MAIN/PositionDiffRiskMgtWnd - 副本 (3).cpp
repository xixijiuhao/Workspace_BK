...
void PositionDiffRiskMgtWnd::QryConfigFinish()
{
	...
	if (m_nInitGroupCfgCnt > 0)
	{
		...
		{
			auto iterFind = m_GroupNoAndCfgDataMap.find(m_MainGroupUserCfg);
			//TODO 存储当前用户的风控配置
			if (iterFind != m_GroupNoAndCfgDataMap.end())
				m_cfgData = iterFind->second;
		}
		else
		{
			auto iterFind = m_GroupNoAndCfgDataMap.find(m_sCurUserInGroupNO);
			//TODO 存储当前用户的风控配置
			if (iterFind != m_GroupNoAndCfgDataMap.end())
				m_cfgData = iterFind->second;
		}
...
void PositionDiffRiskMgtWnd::OnButtonClickDown(WPARAM wParam, LPARAM lParam)
{
	...
	{
		ListView_GetItemText(m_ListGroupInfo.GetListHwnd(), 0, 1, sText, sizeof(sText) / sizeof(sText[0]));
		m_CurGroupNO = LoadRC::unicode_to_ansi(sText).c_str();
	}
...
void PositionDiffRiskMgtWnd::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	m_CurGroupNO = m_ComboxCurUser.GetText();
	if (m_CurGroupNO == "")
	{
	...
	{
		m_GroupNoAndCfgDataMap[m_CurGroupNO].m_bNeedLessPosiRag = m_CheckboxHangupOrd.GetValue();
	...
	{
		m_GroupNoAndCfgDataMap[m_CurGroupNO].m_bIsCommonModel = m_CheckboxCancelType.GetValue();
	...
	else if(hwnd == m_ComboxCurUser.GetHwnd())
	{
		...
		ConfigData cfgData;
		if (m_CurGroupNO != "")
			cfgData = m_GroupNoAndCfgDataMap[m_CurGroupNO];
		ChangeFrmWithGroup(cfgData);
	}
	else if (hwnd == m_ComboxCurUserGroup.GetHwnd())
	{
		m_CurGroupNO = m_ComboxCurUserGroup.GetText();
		//添加切换分组的界面操作
		ConfigData cfgData;
		if (m_CurGroupNO != "")
			cfgData = m_GroupNoAndCfgDataMap[m_CurGroupNO];
...
void PositionDiffRiskMgtWnd::OnBtnAdd()
{
	...
	ShowList(m_GroupNoAndCfgDataMap[m_CurGroupNO]);
}

void PositionDiffRiskMgtWnd::DealWithContractAddData(string &strContractNO, int nRange)
{
	auto iterContract = m_GroupNoAndCfgDataMap[m_CurGroupNO].m_ContractPosiRangeMap.find(strContractNO);
	if (iterContract != m_GroupNoAndCfgDataMap[m_CurGroupNO].m_ContractPosiRangeMap.end())
	{
		//如果有重复的则需要先删除再增加到服务器
	...
	if (iterCompare != m_GroupInfoCompareMap.end())
	{
		...
		for (auto iterGroupInfo = vecIndexGroupInfo.begin(); iterGroupInfo != vecIndexGroupInfo.end(); ++iterGroupInfo)
		{
				...
				strcmp(cfgGroupRsp.ValueStr, strContractNO.c_str()) == 0)
			{
				...
				string sGroupNO = cfgGroupRsp.GroupNo;
				SaveGroupNoAndIndexMap(sGroupNO, curKeyIndex);
				break;
			}
		}
...
void PositionDiffRiskMgtWnd::SetCurGroupSigInfoToAPI(string &strKey,int valueInt)
{
	...
	if (iterCompare != m_GroupInfoCompareMap.end())
	{
		...
		{
			++m_GroupNoAndIndexCnt[m_CurGroupNO];
			curKey = m_GroupNoAndIndexCnt[m_CurGroupNO];
		}
		//重新插入所有非合约和非仓差的其他配置
		strcpy_s(cnfGroupAdd.UserNo, m_CurUserNO.c_str());
		strcpy_s(cnfGroupAdd.GroupNo, m_CurGroupNO.c_str());
...
void PositionDiffRiskMgtWnd::DealGroupInfoToCompare(ConfigGroupRsp &cfgGroupData)
{
	...
	SaveGroupNoAndIndexMap(nGroupNo,nKeyIndex);
}

void PositionDiffRiskMgtWnd::SaveGroupNoAndIndexMap(string &nGroupNo, int nKeyIndex)
{
...
void PositionDiffRiskMgtWnd::SetMainGroupNOToAPI()
{
	...
	//TODO 更新当前的分组的数据
	auto iter = m_GroupNoAndCfgDataMap.find(m_ComboxCurUserGroup.GetText());
	if (iter != m_GroupNoAndCfgDataMap.end())
