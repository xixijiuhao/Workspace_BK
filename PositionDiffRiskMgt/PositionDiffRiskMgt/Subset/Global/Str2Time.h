...
//
static void CharToWChar(wchar_t * Dest, char * Souc)
{
	int nLen = strlen(Souc) + 1; 
	int nwLen = MultiByteToWideChar(CP_ACP, 0, Souc, nLen, nullptr, 0); 

	MultiByteToWideChar(CP_ACP, 0, Souc, nLen, Dest, nwLen); 
}
static void WCharToChar(char * Dest, wchar_t * Souc)
{
	int nLen = wcslen((wchar_t *)Souc)+1;    
	WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)Souc, nLen, (LPSTR)Dest, 2*nLen, nullptr, nullptr);
}
//时区全称
static void GetStandardName(char & Name)
{
	...
	WCharToChar(&Name, (wchar_t *)&(tmInfo.StandardName));
}
//时区简称
static void GetDaylightName(char & Name)
{
	...
	WCharToChar(&Name, (wchar_t *)&(tmInfo.DaylightName));
}
//字符串前n位比较(不分大小写)
