...
void TCheckBox::OnPaint()
{
	...
	rcsel.left = rcsel.right + 2; rcsel.right = rect.right - 2;
	...
	if (m_bfocus)
	{
		...
		rcdot.left = rcsel.left -1; rcdot.right = rcdot.left + sizetext.cx + 1;

		memdc.DotRectangle(rcdot, m_fgclr, 1);
	}

	DrawText(memdc.GetHdc(), LoadRC::ansi_to_unicode(sztext).c_str(), sztext.length(), &rcsel, DT_LEFT | DT_VCENTER);

	DrawText(memdc.GetHdc(), LoadRC::ansi_to_unicode(m_stcheck.sztext2).c_str(), m_stcheck.sztext2.length(), &rcsel, DT_RIGHT | DT_VCENTER);
