...
void TAccessAlarm::DrawMain(TMemDC* pmemdc)
{
	...
	{
		DrawText(pmemdc->GetHdc(), L"认证未通过，请联系易盛市场部：021-50196565授权。", -1, &rc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
	}
	else if (m_iFrmType == userTimeOut)
	{
		DrawText(pmemdc->GetHdc(), L"授权到期，请联系易盛市场部：021-50196565授权。", -1, &rc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
	}
	else if (m_iFrmType == daysLeftWarn)
	{
		...
		rc.top += widgetHeight;
		rc.bottom = rc.top + widgetHeight;
		DrawText(pmemdc->GetHdc(), L"为不影响您的使用，请联系易盛市场部021-50196565授权。", -1, &rc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
	}
}
void TAccessAlarm::OnCreate()
{
	...
	TStaticButton::CreateButton(m_Hwnd,m_btnOk, "关闭窗口", 0);
