...
void TGridCtrl::DrawRow(TMemDC* pmemdc, RECT& rcrow, int nrow)
{
	...
	for (int ncol = m_nfixcolumn; ncol < (int)m_ncolumncount; ncol++)
	{
		...
		if (rccell.left < rcrow.left + nfixwidth)
			rccell.left = rcrow.left + nfixwidth;
...
void TGridCtrl::SetItem(GV_ITEM* pitem)
{
	...
	cpdata->b_top = pitem->b_top;
	cpdata->b_left = pitem->b_left;
	cpdata->b_bottom = pitem->b_bottom;
	cpdata->b_right = pitem->b_right;
	return;
}
...
void TGridCtrl::GetNonfixDataRange(DataRange& strange, bool bresetpos/* = false*/)
{
	...
	strange.nendrow = min(m_vertscroll.nPos + nvisablerow, (_uint)m_vertscroll.nMax - /*m_nfixrow*/1);
	strange.nbegincol = 0;
	strange.nendcol = 7;
}
