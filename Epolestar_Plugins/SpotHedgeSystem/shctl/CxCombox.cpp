...
{
	HWND hcb = CreateWindowEx(0, WC_COMBOBOX, TEXT(""), WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST | CBS_HASSTRINGS | WS_VSCROLL | WS_CLIPSIBLINGS | WS_TABSTOP | CBS_AUTOHSCROLL,
		0, 0, 0, 0, hParent, (HMENU)(id), GetModuleHandle(NULL), NULL);
	Init(hcb);
	SetFont(g_FontData.GetFontWord13());
}
...
int CxCombox::GetCount()
{
	return SendMessage(m_hWnd, CB_GETCOUNT, 0, 0);
}

bool CxCombox::SetSelect(const TCHAR *pszText)
{
...
void CxCombox::GetText(TCHAR* pszText)
{
	int index = GetSelect();
	SendMessage(m_hWnd, CB_GETLBTEXT, index, (LPARAM)pszText);
}
void CxCombox::Clear()
{
