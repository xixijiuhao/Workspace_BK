...
void  CxCheck::Init(HWND hwnd, COLORREF color)
{
	...
	SIZE sz;
	::GetTextExtentPoint32(hDC, strText.c_str(), (int)wcslen(strText.c_str()), &sz);
	DeleteDC(hDC);
	RECT rectCheck;
...
LRESULT CALLBACK CxCheck::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSELEAVE:
    {
        ...
        SendMessage(m_hParenthWnd, WM_USER_MOUSELEAVE, GetWindowLong(GetHwnd(), GWL_ID), 0);
        //m_bInRect = false;
        break;
    }
	case WM_MOUSEHOVER:
		m_bHover = true;
		InvalidateRect(GetHwnd(), 0, TRUE);
        SendMessage(m_hParenthWnd, WM_USER_MOUSEHOVER, GetWindowLong(GetHwnd(), GWL_ID), 0);
		break;
	case WM_SETFOCUS:
		m_bFocus = true;
		InvalidateRect(GetHwnd(), 0, TRUE);
...
void CxCheck::OnPaint()
{
	...
	DrawText(memdc, strText.c_str(), wcslen(strText.c_str()),&rcsel,DT_SINGLELINE|DT_LEFT|DT_VCENTER);
