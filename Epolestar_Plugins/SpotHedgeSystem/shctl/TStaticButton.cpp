...
TStaticButton::TStaticButton()
{
	...
	m_DTStyle = DT_CENTER;
	m_font = g_FontData.GetFontWord13();
	m_sztext = "È·¶¨";
	m_gradient = NULL;
	_GradientFill = NULL;
	m_bfocus = false;
...
void TStaticButton::Create(HWND hParent, const char *pText, int id)
{
	Create(hParent, id);
...
void TStaticButton::OnPaint()
{
	...
	if (m_bClkChange)
	{
		...
		if (m_bmouseover)
		{
			...
			FillRect(memdc.GetHdc(), &rect, hbr);
			DeleteObject(hbr);
			SetTextColor(memdc.GetHdc(), g_ColorRefData.GetColorWhite());
		...
		if (!m_benable)
		{
			...
			//FillRect(memdc.GetHdc(), &rect, g_ColorRefData.GetHbrGray());
			//DeleteObject(hbr);
	...
	DrawTextA(memdc.GetHdc(), m_sztext.c_str(), m_sztext.length(), &rect, m_DTStyle);
