        ...
        if (nContract > 0)
        {
            ...
            if (nretCon <= LINE_VALID || IsSpreadType(m_CurVarietyType))
            {
                for (int i = 0; i < nretCon; i++)
                {
					...
					//CConvert::SContractNoToTContractNo(sContractNo, strContract);
...
void CPopWinCode::SetTargetCommodity(string commodityNo)
{
	m_commodityNo = commodityNo;
}
    ...
    for (size_t i = 0; i < vSortedExchange.size(); i++)//遍历交易所 交易所数量等于1自动展开
    {
        ...
        if (g_pStarApi->GetExchangeData(strExchange, &pExchange, 1, false) && pExchange)
        {
            ...
            for (size_t j = 0; j < commodityData.size(); j++)
            {
                ...
                {
					if (!m_commodityNo.empty())
					{
						if (strstr(pCommodity->CommodityNo, m_commodityNo.c_str()) != NULL)
						{
            ...
            //int nretCom = g_pStarApi->GetCommodityData(pExchange->ExchangeNo, "", sCommodity, nCommodity, true);
