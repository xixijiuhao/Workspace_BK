...
//zyp 新增处理投机check
void COFInputDlg::OnBnClickedCheckSP()
{
	if (m_CheckSP.GetCheck() == BST_UNCHECKED)
	{ 
		m_CheckHG.SetCheck(BST_UNCHECKED);
		m_CheckSP.SetCheck(BST_CHECKED); 
	}
	else
	{
		m_CheckSP.SetCheck(BST_UNCHECKED);
	}
	ClearTipBar();
}

//zyp 新增处理保值check
void COFInputDlg::OnBnClickedCheckHG()
{
	if (m_CheckHG.GetCheck() == BST_UNCHECKED)
	{
		m_CheckHG.SetCheck(BST_CHECKED);
		m_CheckSP.SetCheck(BST_UNCHECKED);
	}
	else
	{
...
bool COFInputDlg::SetCtrlFont()
{
	...
	//zyp 新增套利/保值
	SendMessage(GetDlgItem(m_hDlg, IDC_CHECK_HG), WM_SETFONT, (WPARAM)m_fontCheck, TRUE);
	SendMessage(GetDlgItem(m_hDlg, IDC_CHECK_SP), WM_SETFONT, (WPARAM)m_fontCheck, TRUE);
...
void COFInputDlg::TJCheck(C_OrderField &of, RECT &ctrlRect)
{
	...
	//套利 根据是否有交易员登陆决定
	CheckLayout(IDC_CHECK_SP, IsShowSPAndHG(), of.height, ctrlRect);
	//套利 根据是否有交易员登陆决定
	CheckLayout(IDC_CHECK_HG, IsShowSPAndHG(), of.height, ctrlRect);
}
//更新状态
...
void COFInputDlg::InitCtrl()
{
	...
	//zyp 新增套利/保值
	SetWindowTextW(GetDlgItem(m_hDlg, IDC_CHECK_SP), LoadResString(IDS_STRING_Speculate).c_str());
	SetWindowTextW(GetDlgItem(m_hDlg, IDC_CHECK_HG), LoadResString(IDS_STRING_Hedge).c_str());
	...
	//zyp 新增 套利/保值
	m_CheckHG.InitBtn(GetDlgItem(m_hDlg, IDC_CHECK_HG));
	m_CheckSP.InitBtn(GetDlgItem(m_hDlg, IDC_CHECK_SP));
	...
	bool bFastOrder, char key, const char * ContractID)
{
	...
	//zyp 新增套利保值
	if (IsShowSPAndHG() && IsCheck(IDC_CHECK_HG))
	{
		SendOrder.Hedge = hHedge;
	}
	else if (IsShowSPAndHG() && IsCheck(IDC_CHECK_SP))
	{
...
void COFInputDlg::ResetTJCheckBox(TBool IsRiskOrder, TTradeSection AddOneIsValid)
{
	...
	if (IsCheck(IDC_CHECK_SP))
		OnBnClickedCheckSP();
	if (IsCheck(IDC_CHECK_HG))
		OnBnClickedCheckHG();
...
bool COFInputDlg::IsCheck(int check)
{
	...
	else if (check == IDC_CHECK_SP) p = &m_CheckSP;
	else if (check == IDC_CHECK_HG) p = &m_CheckHG;
...
void COFInputDlg::UpdateAltHotKeyShowState()
{
	...
	if (!m_bHotKeyAltActive)
	{
		...
		SetWindowTextW(m_CheckSP.GetHwnd(), LoadResString(IDS_STRING_Speculate).c_str());
		InvalidateRect(m_CheckSP.GetHwnd(), NULL, TRUE);
		SetWindowTextW(m_CheckHG.GetHwnd(), LoadResString(IDS_STRING_Hedge).c_str());
		InvalidateRect(m_CheckHG.GetHwnd(), NULL, TRUE);
...
void COFInputDlg::SetCodeFromQuote(const char * sKey, const char* cType)	//双击 单击 行情填单
{
	...
	if (vecNoticeContent[ID_SRC].strContent == W_SRC_GRID)
	{
		if (!strcmp(cType, W_LButtonDown))
		{
			...
			{
				if (m_KeyBoardSet.OutPriceType == 4)
				{
					m_FillOrderInfo.cDirect = ((m_FillOrderInfo.cDirect == dSell) ? dBuy : dSell);
				}
...
void COFInputDlg::UpdatePropertyCfg()
{
	...
	sprintf_s(cProperty, "ConditionCheck=%d;BackHandCheck=%d;FastCheck=%d;BatchCountCheck=%d;AddOne=%d", 
		m_CheckTJ.GetCheck() == BST_CHECKED ? 1 : 0, 
		m_CheckPK.GetCheck() == BST_CHECKED ? 1 : 0,
		m_CheckKS.GetCheck() == BST_CHECKED ? 1 : 0, 
		m_CheckPC.GetCheck() == BST_CHECKED ? 1 : 0,
		m_CheckT1.GetCheck() == BST_CHECKED ? 1 : 0);
