...
{
	if (m_opr)
		m_opr->SetContractNo(m_sCode.c_str(), true, true);
...
//        sprintf_s(sKey, "%s$%c", pCommodity->CommodityNo, Goods_Type);
//    else if (ctDefer == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->CommodityNo, Defer_Type);
//    else if (ctFutures == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->CommodityNo, Future_Type);
//    else if (ctSpreadMonth == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->TargetCommodity1->CommodityNo, SpreadMonth_Type);
//    else if (ctSpreadCommodity == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c$%s", pCommodity->TargetCommodity1->CommodityNo, SpreadCommodity, pCommodity->TargetCommodity2->CommodityNo);
//    else if (ctOption == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->TargetCommodity1->CommodityNo, Option_Type);
//    else if (ctBUL == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->TargetCommodity1->CommodityNo, BUL_Type);
//    else if (ctBER == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->TargetCommodity1->CommodityNo, BER_Type);
//    else if (ctSTD == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->TargetCommodity1->CommodityNo, STD_Type);
//    else if (ctSTG == pCommodity->CommodityType)
//        sprintf_s(sKey, "%s$%c", pCommodity->TargetCommodity1->CommodityNo, STG_Type);
//
//    strKey = sKey;
//}
...
void CPopWinCode::UpdateContract(int Type)
{
    ...
    {
        string sCommodityNo = m_strCurExchange + "|" + CConvert::TradeTypeToQuoteType(m_CurVarietyType) + "|" + m_strCurCommodity;
        int nOptionSerino = g_StarApi->GetContractCount(sCommodityNo.c_str());
        UpdateListCtrl(nOptionSerino);
        if (nOptionSerino>0)
        {
            SContract **tOContract = new SContract*[nOptionSerino];
            nOptionSerino = g_StarApi->GetContractData(sCommodityNo.c_str(), "", tOContract, nOptionSerino, true);
...
bool CPopWinCode::UpdateData(char * cSign)
{
    ...
    for (size_t i = 0; i < vSortedExchange.size(); i++)//遍历交易所 交易所数量等于1自动展开
    {
        ...
        if (g_StarApi->GetExchangeData(strExchange, &pExchange, 1, false) && pExchange)
        {
            ...
            for (size_t j = 0; j < commodityData.size(); j++)
            {
                ...
                if (g_StarApi->GetCommodityData(pExchange->ExchangeNo, strCommodity, &pCommodity, 1, false) && pCommodity)
                {
            ...
            //SCommodity **sCommodity = new SCommodity*[nCommodity];
            //int nretCom = g_StarApi->GetCommodityData(pExchange->ExchangeNo, "", sCommodity, nCommodity, true);

            ////品种排序
            //
            //for (int j = 0; j < nretCom; j++)
            //{
            //    SCommodity *Commodity = (SCommodity *)sCommodity[j];
            //    if (!Commodity)
            //        break;
            ...
            for (size_t j = 0; j < compareData.size(); j++)
            {
                ...
                {
                    wchar_t *p = wcsstr(wCommodityName, LoadResString(IDS_Spread).c_str());
                    wchar_t *q = wcsstr(wCommodityName, LoadResString(IDS_SpreadM).c_str());
                    if (!p && !q)
                        swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName, LoadResString(IDS_Spread));
                    else
                ...
                {
                    wchar_t *p = wcsstr(wCommodityName, LoadResString(IDS_Option).c_str());
                    if (!p)
                        swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName, LoadResString(IDS_Option));
                    else
                        swprintf_s(strCommodity, L"%s[%s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName);
                }
                else if ((IsOpthinCmbType(tkey.CommodityType)))
                {
                    ...
                    {
                        wchar_t *p = wcsstr(wCommodityName, LoadResString(IDS_Straddle).c_str());
                        if (!p)
                            swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName, LoadResString(IDS_Straddle));
                        else
                            swprintf_s(strCommodity, L"%s[%s] ", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName);
                    }
                    else if (tkey.CommodityType == ctSTG)
                    {
                        wchar_t *p = wcsstr(wCommodityName, LoadResString(IDS_Strangle).c_str());
                        if (!p)
                            swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName, LoadResString(IDS_Strangle));
                        else
                            swprintf_s(strCommodity, L"%s[%s]  ", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName);
                    }
                }
                ...
                {
                    int nContract = g_StarApi->GetOptionSeriesCount(pCommodity->CommodityNo);
                    if (nContract <= 0)
                    {
                        continue;
                    }
                    ...
                    int retCon = g_StarApi->GetOptionSeriesData(pCommodity->CommodityNo, "", tContract, nContract, true);
                ...
                else if (IsOpthinCmbType(tkey.CommodityType))
                {
                    ...
                    int nContract = g_StarApi->GetOptionSeriesCount(sCommodityNo);
                    if (nContract <= 0)
                    {
                        continue;
                    }
                    ...
                    SOptionSeries **tContract = new SOptionSeries*[nContract];
                    int retCon = g_StarApi->GetOptionSeriesData(sCommodityNo, "", tContract, nContract, true);
                    for (int k = 0; k < retCon; k++)
                    {
                        SOptionSeries *Contract = (SOptionSeries *)tContract[k];
                        if (!Contract)
	...
	//g_pTradeData->GetAllExchange(tempExcDataCon, cSign);
	...
	//	swprintf_s(strExchange, L"%s[%s]", LoadRC::ansi_to_unicode(pExchange->ExchangeNo).c_str(), pExchange->ExchangeName);
	//	//根据品种类型获得所有商品
	//	tempComData.clear();
	...
	//		
	//		wchar_t strCommodity[60];
	//		if (IsSpreadType(pCommodity->CommodityType))
	//		{
	//			wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResString(IDS_Spread));
	//			wchar_t *q = wcsstr(pCommodity->CommodityName, LoadResString(IDS_SpreadM));
	//			if (!p&&!q)
	//				swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResString(IDS_Spread));
	//			else
	//				swprintf_s(strCommodity, L"%s[%s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);
	//				
	//		}
	//			
	//		else if (IsOptionType(pCommodity->CommodityType))
	//		{
	//			wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResString(IDS_Option));
	//			if (!p)
	//				swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResString(IDS_Option));
	//			else
	//				swprintf_s(strCommodity, L"%s[%s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);					
	//		}
	//		else if ((IsOpthinCmbType(pCommodity->CommodityType)))
	//		{
	//			if (pCommodity->CommodityType == ctSTD)
	//			{
	//				wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResString(IDS_Straddle));
	//				if (!p)
	//					swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResString(IDS_Straddle));
	//				else
	//					swprintf_s(strCommodity, L"%s[%s] ", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);
	//			}
	//			else if (pCommodity->CommodityType == ctSTG)
	//			{
	//				wchar_t *p = wcsstr(pCommodity->CommodityName, LoadResString(IDS_Strangle));
	//				if (!p)
	//					swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName, LoadResString(IDS_Strangle));
	//				else
	//					swprintf_s(strCommodity, L"%s[%s]  ", LoadRC::ansi_to_unicode(pCommodity->CommodityNo).c_str(), pCommodity->CommodityName);
	//			}
	//		}
