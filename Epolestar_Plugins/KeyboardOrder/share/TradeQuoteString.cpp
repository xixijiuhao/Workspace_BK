...
bool CConvert::GetQuoteCommodityInf(const char*code, SOFCommodityInf & sCommodityInfo)
{
	...
	int num = g_StarApi->GetCommodityData("", QCommodityNo, &tmpCommodity, 1,false);
...
{
	SContract* out = NULL;
	if (g_StarApi->GetContractData("", ContractId, &out, 1, false))
		return true;
	else
		return false;
}
...
bool CConvert::IsValidSpreadContract(const  char * ContractId)
{
    ...
    if (g_StarApi->GetSpreadInfo(ContractId, &sQuote, 1, false))
    {
        return true;
    }
    return false;
...
{
    //只返回一档行情数据
	if (!qContract || !g_StarApi)
		return 0;
	if (nField == S_FID_BESTBIDPRICE || nField == S_FID_BESTASKPRICE)
	{
		SQuoteField qInfo;
		if (g_StarApi->GetImpliedField(qContract, nField, &qInfo))
