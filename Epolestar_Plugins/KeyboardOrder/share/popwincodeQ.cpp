...
const char* FIRST_EXCHANGE_ARRAYQ[] = {
...
LRESULT CALLBACK ListViewProcQ(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	int id = GetWindowLong(hwnd, GWL_ID);
	CPopWinCodeQ *pWinCode = (CPopWinCodeQ*)GetWindowLong(hwnd, GWL_USERDATA);
	if (!pWinCode)
		return false;
	switch (message)
	{
...
void CPopWinCodeQ::CreatePopWin(IOperator* opr,LPCTSTR classname, HWND parent, DWORD style, DWORD exstyle, int nleft, int ntop, UINT nWidth, UINT nrow)
{
	m_nWidth = nWidth;
	m_nRow = nrow;
	m_nLeft = nleft;
...
void CPopWinCodeQ::Less_Commodity::GetKey(const SCommodity* pCommodity, string &strKey)
{
	char sKey[20] = { 0 };
...
//void CPopWinCodeQ::Commodity_Compare::GetKey(const CommodityAndKey * data, std::string &strKey)
//{
//    char sKey[20] = { 0 };
...
bool CPopWinCodeQ::IsOptionType(char CommodityType)
{
	return CommodityType == ctOption;
}

bool CPopWinCodeQ::IsOpthinCmbType(char CommodityType)
{
	return ctBUL == CommodityType || ctBER == CommodityType || ctSTD == CommodityType || ctSTG == CommodityType;
}

bool CPopWinCodeQ::IsSpreadType(char CommodityType)
{
	return ctSpreadMonth == CommodityType || ctSpreadCommodity == CommodityType;
}

bool CPopWinCodeQ::IsLMEFutures(const char *szExchange, const char *szSign)
{
...
//void CPopWinCodeQ::GetContractByOption(TDataContainer & ConDataCon, set<string> &setContract)
//{
...
void CPopWinCodeQ::UpdateListCtrl(uint nrow)
{
	ClearListCtrl();
	int nxO = 85;
	int nxF = 170;
...
void CPopWinCodeQ::DelLParamPtr()
{
...
char* CPopWinCodeQ::GetParam(const char *szInput, char CommodityType)
{
...
void CPopWinCodeQ::TravelItem( HTREEITEM hItem)
{
...
bool CPopWinCodeQ::GetItemByH(HTREEITEM hItem, TVITEM & item)
{
	memset(&item, 0, sizeof(item));
	item.hItem = hItem;
	item.mask = TVIF_TEXT | TVIF_HANDLE | TVIF_PARAM;
...
void GetSignExchangeQ(const char *szParam, string & strEx, string &strSign)
{
	string str(szParam);
	int index = str.find(' ');
	if (index > 0)
...
int  CPopWinCodeQ::GetTreeParam(HTREEITEM hItem)
{
	//清空数据
	...
	{
		GetSignExchangeQ(vecStr[0].c_str(), m_strCurExchange, m_strSign);	
	}
	else if (type == CommoditySele_Type)
	{
		...
		GetSignExchangeQ(vecStr[1].c_str(), m_strCurExchange, m_strSign);
	...
	else if (type == ContractSele_Type)
	{
		...
		GetSignExchangeQ(vecStr[2].c_str(), m_strCurExchange, m_strSign);
	}
	return type;
}

void CPopWinCodeQ::ClearListCtrl()
{
	ListView_DeleteAllItems(m_hListView);//清行
	while (ListView_DeleteColumn(m_hListView, 0));//清列
}

void CPopWinCodeQ::UpdateContract(int Type)
{
	ClearListCtrl();
...
bool CPopWinCodeQ::UpdateData()
{
	...
	for (int i=0;i<sizeof(FIRST_EXCHANGE_ARRAYQ)/ sizeof(char*); i++)
		vExchange.push_back(FIRST_EXCHANGE_ARRAYQ[i]);
		...
		if (g_StarApi->GetExchangeData((*iter).c_str(), &pExchange, 1, false) && pExchange)
		{
			...
			for (int j = 0; j < compareData.size(); j++)
			{
				...
				if (IsOptionType(tkey.CommodityType))
				{
					...
					if (!p)
						swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName, LoadResString(IDS_Option).c_str());
					else
...
LRESULT CPopWinCodeQ::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	HDC			hdc;
	switch (message)
	{
	case WM_CREATE:
		{
			...
			m_ListViewProc = (WNDPROC)SetWindowLong(m_hListView, GWL_WNDPROC, (LONG)ListViewProcQ);
