...
void TPointOrderFrm::SetContractNo(string sCode)
{
	...
	m_clData.SetContractNo(sCode, m_stOrderInfo.szUserNo, sizeof(m_stOrderInfo.szUserNo));
	m_sCode = sCode;
	...
	if (stUser.size())
	{
		...
		if (pUser->pLogin && pUser->pLogin->Logined == bYes)
		{
			if (!strcmp(pUser->pLogin->LoginClassName, LoadRC::unicode_to_ansi(PLUG_FOREIGNTRADEAPI_NAME).c_str()) || 
				!strcmp(pUser->pLogin->LoginClassName, LoadRC::unicode_to_ansi(PLUG_DIPPERTRADEAPI_NAME).c_str()))
				m_bIsForeign = true;
		}
	}
	//根据内盘，外盘和语言设置来重置布局；
...
void TPointOrderFrm::DrawMainData(TMemDC &memdc)
{
	...
	SQuoteSnapShotL2 buyQuoteDataL2;
	SQuoteSnapShotL2 sellQuoteDataL2;
	m_clData.GetBuyAndSellQuoteDataL2(buyQuoteDataL2,sellQuoteDataL2);

	dLastPrice = QuoteData.Data[S_FID_LASTPRICE].Price;
	if (fabs(dLastPrice) < ErrorRange)
		dLastPrice = QuoteData.Data[S_FID_PRESETTLEPRICE].Price;
	...
	for (int iCol = 0; iCol < 9; iCol++)
	{
		...
		for (int iRow = 0; iRow < m_iVisiabalRow; iRow++)
		{
			...
			int iMouseCol = GetCol(LOWORD(m_MouseMoveOffset));
 			if (oldCol == iMouseCol && 
				iRow == HIWORD(m_MouseMoveOffset) / iWight_Height && 
				m_bMouseTrack && 
				iCol != ID_COL_LONGPOSITION && 
				iCol != ID_COL_SHORTPOSITION && 
				iCol != ID_COL_ORDERPRICE)
 			{
			...
			case ID_COL_LONGORDERQTY:
			{
				...
				//根据鼠标颜色填充价格
				if (oldCol == GetCol(LOWORD(m_MouseMoveOffset)) && 
					iRow == HIWORD(m_MouseMoveOffset) / iWight_Height && 
					m_bMouseTrack && 
					iCol != ID_COL_LONGPOSITION && 
					iCol != ID_COL_SHORTPOSITION && 
				...
				{
					if (fabs(price - QuoteData.Data[S_FID_BESTBIDPRICE + iCol].Price) < ErrorRange)
					{
						iQty = QuoteData.Data[S_FID_BESTBIDQTY + iCol].Qty;
						break;
					}
				}*/
				for (int i = 0;i < MAX_L2_DEPTH;++i)
				{
					if (fabs(price - buyQuoteDataL2.Data[i].Price) < ErrorRange)
					{
						iQty = buyQuoteDataL2.Data[i].Qty;
						break;
					}
				}
			...
			case ID_COL_SHORTORDERQTY:
			{
				...
				/*for (int iCol = 0; iCol < 20; iCol++)
				{
					if (fabs(price - QuoteData.Data[S_FID_BESTASKPRICE + iCol].Price) < ErrorRange)
					{
						iQty = QuoteData.Data[S_FID_BESTASKQTY + iCol].Qty;
						break;
					}
				}*/
				...
				for (int i = 0; i < MAX_L2_DEPTH; ++i)
				{
					if (fabs(price - sellQuoteDataL2.Data[i].Price) < ErrorRange)
					{
						iQty = sellQuoteDataL2.Data[i].Qty;
						break;
					}
				}
				if (iQty > 0)
...
void TPointOrderFrm::SetQuoteHeaderInfo()
{
	...
	/*for (int iOffset = 0; iOffset < 20; iOffset++)
	{
		m_stGridHeaderInfo.iBuyQuoteQty += QuoteData.Data[S_FID_BESTBIDQTY + iOffset].Qty ;
		m_stGridHeaderInfo.iSellQuoteQty += QuoteData.Data[S_FID_BESTASKQTY + iOffset].Qty;
	}*/

	for (int iOffset = 0; iOffset < MAX_L2_DEPTH;++iOffset)
	{
		m_stGridHeaderInfo.iBuyQuoteQty += buyQuoteDataL2.Data[iOffset].Qty;
		m_stGridHeaderInfo.iSellQuoteQty += sellQuoteDataL2.Data[iOffset].Qty;
	}
}
