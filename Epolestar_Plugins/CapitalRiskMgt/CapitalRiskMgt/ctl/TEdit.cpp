...
void TEdit::CreateEdit(HWND hParent, TEdit &edit, int style, int index, DWORD specialstyle)
{
	edit.m_nid = style;
	edit.Create(hParent, specialstyle, index);
...
double TEdit::GetWindowDoubleNum()
{
	char szText[50] = { 0 };
	GetWindowText(szText, 50);
	return atof(szText);
...
void TEdit::OnPaint()
{
	...
	if (m_benable)
	{
    	FillRect(memdc.GetHdc(), &rect, g_ColorRefData.g_HbrWhite);
	}
	else
	{
...
LRESULT TEdit::OnCtlColor(WPARAM wParam, LPARAM lParam)
{
	if (m_benable)
	{
		...
		return (LRESULT)g_ColorRefData.g_HbrWhite;
	}
	SetBkColor(HDC(wParam), g_ColorRefData.g_ColorGray);
	return (LRESULT)g_ColorRefData.g_HbrGray;
}
...
void  TEdit::OnKeyDown(WPARAM wParam, LPARAM lParam, HWND hparent)
{
	if (VK_TAB == (int)wParam)
		...
		SendMessage(GetParent(), SSWM_EDIT_VK_RETURN, m_nid, (LPARAM)m_Hwnd);
...
LRESULT TEdit::EditProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	case WM_CHAR:  //限制输入浮点数据,暂时先这样处理，后面在优化，重点是画界面
		...
		else if (uIdSubclass == TEdit::TEDIT_SINT)
		{
			...
			if (wParam == '+' || wParam == '-')
			{
				DWORD dWord =  Edit_GetSel(hwnd);
				int iStart = LOWORD(dWord);
				int iEnd = HIWORD(dWord);

				GetWindowTextA(hwnd, strTemp, 30);
				if (strlen(strTemp) != 0 && (iEnd - iStart) != strlen(strTemp))
					return 0;
			}
			else if ((wParam<'0' || wParam>'9') && wParam != VK_BACK)
				return 0;
