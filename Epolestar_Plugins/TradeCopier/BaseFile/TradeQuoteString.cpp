...
vector<uint> CConvert::CoverPos(const TPosition* pPosition, TSendOrder & stOrder, int coverQty)
{
	...
	else
	{
		...
		if (dayQty >= coverQty) //如果今仓的数量已经满足平仓量
		{
		...
		else //如果不满足，则先把进仓平掉
		{
			...
			if (pPosition->PrePositionQty)//今仓平仓数量不够用，需要平昨仓
			{
				int preQty = coverQty - dayQty;//减去今仓之后还需要平的数量
											   //还需要平的数量为preQty,如果昨仓数量大于preQty,那么平仓数量就是preQty，如果昨仓不够则把昨仓都平掉
