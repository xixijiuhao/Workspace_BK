...
void TClickPriceOrderOptionWnd::Create(HWND hParent)
{
	if (!IsWindow(m_Hwnd))
	{
		m_hParent = hParent;
...
void TClickPriceOrderOptionWnd::MoveWindow(int x, int y)
{
	SetWindowPos(m_Hwnd, 0, x, y, FRAME_V, FRAME_H, SWP_NOZORDER);
	InitRcClose();
}
void TClickPriceOrderOptionWnd::RegistConfig(PointOrderConfig* stConfig)
{
	if (!stConfig)
		return;
	m_pstConfig = stConfig;
}
LRESULT TClickPriceOrderOptionWnd::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_PAINT:
...
void TClickPriceOrderOptionWnd::OnPaint()
{
	TMemDC memdc(m_Hwnd);
...
void TClickPriceOrderOptionWnd::OnCreate()
{
	CreateWidget();
	InitWidget();
	SetWidgetPos();
...
void TClickPriceOrderOptionWnd::OnSize(WPARAM wParam, LPARAM lParam)
{

}
LRESULT TClickPriceOrderOptionWnd::OnCommand(WPARAM wParam, LPARAM lParam)
{
...
void TClickPriceOrderOptionWnd::OnDealPriceType()
{
	int index = SendMessage(m_cbCoverType.GetHwnd(), CB_GETCURSEL, 0, 0);
	if (index != CB_ERR)
		m_pstConfig->iCoverAllPriceType = index;
}
LRESULT TClickPriceOrderOptionWnd::OnCtrlColor(WPARAM wParam, LPARAM lParam)
{
	SetBkMode((HDC)wParam, TRANSPARENT);
	return (LRESULT)g_ColorRefData.g_brush_white;
}
void TClickPriceOrderOptionWnd::OnLButtonDown(WPARAM wParam, LPARAM lParam)
{
	RECT rect;
...
void TClickPriceOrderOptionWnd::CreateWidget()
{
...
void TClickPriceOrderOptionWnd::InitWidget()
{
...
void TClickPriceOrderOptionWnd::HideSomeConfig()
{
...
void TClickPriceOrderOptionWnd::SetWidgetPos()
{
	const int ileft = 15;
	const int itop = 35;
...
void TClickPriceOrderOptionWnd::InitRcClose()
{
...
void TClickPriceOrderOptionWnd::DrawClose(TMemDC *pMemDC)
{
