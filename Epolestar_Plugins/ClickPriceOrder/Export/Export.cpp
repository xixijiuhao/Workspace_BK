...
G_UIFONT  g_FontData;
TClickPriceOrderWnd* g_pClickPriceOrder = nullptr;
TClickPriceOrderOptionWnd* g_pTClickPriceOrderOptionWnd = nullptr;
TClickPriceOrderTipsFrm g_clTips = TClickPriceOrderTipsFrm::GetInstance();
...
extern "C"
{
	__declspec(dllexport) void __cdecl OnPluginInit(TPluginMgrApi* p)
	{
		...
		g_pMainFrame->RegMenuItem(MAKE_MENUORDER(0, 10002, 2), _GetAppNameString() , TClickPriceOrderWnd::OnMenuClick);
        g_pMainFrame->RegHotKey(false, false, false, VK_F8, false, TClickPriceOrderWnd::TOnHotKeyDown);
