...
{
	MSG  msg;
	if (PeekMessage(&msg, 0, 0, 0, PM_NOREMOVE))			//PM_NOREMOVE    PeekMessage处理后，消息不从队列里除掉。
	{
...
{
	memset(&m_curCurrency, 0, sizeof(SCurrency));
}
...
{
	if (m_opr)
		m_opr->SetContractNo(m_sCode,&m_curCurrency);
...
void CPopWinCode::UpdateContract(int Type)
{
	...
	if (m_CurVarietyType!= ctOption)
	{
		...
		map<string, SCurrency>::iterator iter = m_mComCurrency.find(sCommodityNo);
		if (iter != m_mComCurrency.end())
			memcpy(&m_curCurrency, &iter->second, sizeof(SCurrency));
		else
			memset(&m_curCurrency, 0, sizeof(SCurrency));
...
bool CPopWinCode::UpdateData(bool bShowDirect)
{
	DelLParamPtr();
		...
		if (g_StarApi->GetExchangeData((*iter).c_str(), &pExchange, 1, false) && pExchange)
		{
			...
			for (int j = 0; j < nretCom; j++)
			{
				...
				if (!Commodity)
					break;
				setOContract.clear();
				TCommodityKey tkey;//交易品种关键字
				CConvert::SContractNoToTCommodityKey("", Commodity->CommodityNo, tkey);
				SCurrency currency;
				if (Commodity->Currency)
					memcpy(&currency, Commodity->Currency, sizeof(SCurrency));
				else
					continue;

				m_mComCurrency.insert(pair<string, SCurrency>(Commodity->CommodityNo, currency));
				...
				{
					wchar_t *p = wcsstr(wCommodityName, CConvert::LoadResString(IDS_Option).c_str());
					if (!p)
						swprintf_s(strCommodity, L"%s[%s %s]", LoadRC::ansi_to_unicode(tkey.CommodityNo).c_str(), wCommodityName, CConvert::LoadResString(IDS_Option).c_str());
					else
				...
				if (tkey.CommodityType != ctFutures&&tkey.CommodityType != ctGoods&&tkey.CommodityType != ctDefer&&tkey.CommodityType != ctOption
					&&(!bShowDirect||(tkey.CommodityType != ctDirect&&tkey.CommodityType != ctInDirect&&tkey.CommodityType != ctCross)))
					continue;
		...
		if ((int)wParam == ID_TREE_VIEW)
		{
			...
			if (NM_CLICK == pNMHDR->code)
			{
				...
				if (hChild ==NULL/*m_TreeCtrl.GetNextSiblingItem(hChild)*/)
				{
					...
					if (bLME || m_CurVarietyType == ctGoods || m_CurVarietyType == ctDefer || m_CurVarietyType ==ctDirect|| m_CurVarietyType == ctInDirect || m_CurVarietyType == ctCross)
					{
						SelectCode();
						ClearListCtrl();
					}
