...
void CxCombox::AddString(TCHAR *pszText, int index)
{
...
void CxCombox::SetSelect(TCHAR *pszText)
{
	int index = SendMessage(m_hWnd, CB_FINDSTRING, 0, (LPARAM)pszText);
	SetSelect(index);
}
void CxCombox::SetHeight(int x, int i)
{
	SendMessage(m_hWnd, CB_SETITEMHEIGHT, i, x);
}