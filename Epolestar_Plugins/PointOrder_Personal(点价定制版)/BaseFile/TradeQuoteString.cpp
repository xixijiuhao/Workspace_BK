...
void CConvert::GetPrice(TContractSingleKey* stContract, TDirect direct, int PriceType, double& price)
{
	...
	const SCommodity * pCommodity = g_pTradeData->GetCommodity(stContract);

	if (pCommodity && g_pStarApi->GetSnapshot(qNo, &QuoteData))
	{
		if (direct == dBuy)
		{
			switch (PriceType)
...
bool CConvert::GetCommodity(SCommodity& com, const TCommodityKey* comkey)
{
	const SCommodity* pstCommodity = g_pTradeData->GetCommodity(comkey);

	if (pstCommodity) 
	{
		memcpy(&com, pstCommodity, sizeof(SCommodity));
		return true;
	}
	return false;
}
