...
{
    if (cont && strcmp(cont->ContractNo, m_QuoteContractNo) == 0 &&
		UpdataQuoteData())
	{
...
{
	return (g_pStarApi->GetSnapshot(m_QuoteContractNo, &m_QuoteData));
}

void TPointOrderFrmData::GetQuoteDataL2(SQuoteSnapShotL2 & buy, SQuoteSnapShotL2 & sell, SImpliedDepthL2& implied)
{
	bool bGet = false;
	if (m_pQuoteContract)
	{
		SRetType ret = g_pStarApi->GetImpliedSnapshot(m_pQuoteContract, &buy, &sell, implied);
		bGet = ret > 0;
	}
	
	if (!bGet)
	{
		memset(&implied, 0, sizeof(SImpliedDepthL2));
		g_pStarApi->GetSnapshotL2(m_QuoteContractNo, &buy, &sell);
	}
...
void TPointOrderFrmData::SetContractNo(string sCode, char* UserNo, TContractKey & stKey)
{
	...
	strcpy_s(m_QuoteContractNo, sQuote);

	g_pStarApi->GetContractData("", sQuote, &m_pQuoteContract, 1, false);
	...
	memcpy(&stKey, &m_stConKey, sizeof(TContractKey));
}
