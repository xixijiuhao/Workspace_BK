...
TCHAR szmsgCHS[2][20] = {L"确定", L"取消"};
TCHAR szmsgCHT[2][20] = { L"確定", L"取消" };
TCHAR szmsgENU[2][20] = { L"OK", L"Cancel" };
TCHAR* GetMessagebox(int i)
{
	switch (g_LanguageID)
	{
	case NONE:
...
void TMessageBox::DrawFocusBtn(TMemDC* pmemdc, int id)
{
	...
	if (id == ID_OK)
	{
		...
		DrawText(pmemdc->GetHdc(), GetMessagebox(0), -1, &m_rcOk, uformat);
		SetTextColor(pmemdc->GetHdc(), g_ColorRefData.g_ColorBlack);
		DrawText(pmemdc->GetHdc(), GetMessagebox(1), -1, &m_rcNo, uformat);
	}
	else if (id == ID_NO)
	{
...
bool _TMessageBox_Domodal(HWND hwnd,const  char* sztitle, const wchar_t* sztext, UINT ntype)
{
