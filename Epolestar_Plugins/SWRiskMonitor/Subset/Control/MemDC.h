...
#define CMemDC CMemoryDC//避免和VS2011自带的CMemDC重名
class CMemDC : public CDC
{
public:
    // constructor sets up the memory DC
    CMemDC(CDC* pDC, BOOL bVReverse = FALSE, RECT * rect = nullptr) 
		: CDC()
    ...
    ~CMemDC()
    {
        if (m_bMemDC) 
        {    
			if(m_bVReverse)//需要纵向翻转
			{
				m_pDC->StretchBlt(m_rect.left,m_rect.top,m_rect.Width(), m_rect.Height(),
					this, m_rect.left, m_rect.Height()+m_rect.top-1,m_rect.Width(),-m_rect.Height(),SRCCOPY);
			}
			else
			{
