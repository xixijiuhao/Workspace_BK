...
SDateType KYEAR = 0;
SDateType KMONTH = 0;
KLineSlice slice = KLine_Day;
SKLineTypeType type = S_KLINE_DAY;
...
std::string KLineUtil::UnixTime2Str(int n)
{
	struct tm tm2;
	char strTime[100] = { 0 };
	time_t now = (time_t)n;
	localtime_s(&tm2, &now);
	strftime(strTime, sizeof(strTime) - 1, "%Y-%m-%d %H:%M:%S", &tm2);
	strTime[sizeof(strTime) - 1] = '\0';
	return strTime;
}

SDateTimeType KLineUtil::UnixTime2QuoteTime(int n)
{
	struct tm tm2;
	char strTime[100] = { 0 };
	time_t now = (time_t)n;
	...
	sprintf_s(strTime, "%4d%02d%02d%02d%02d%02d%03d", (tm2.tm_year + 1900), (tm2.tm_mon + 1), tm2.tm_mday, 0, 0, 0, 0);

	return atoll(strTime);
}
...
bool KLineUtil::IsSameMonthTime(SHisQuoteData d)
{
	...
	SDateType year = date / 10000;
	SDateType month = date / 100 % 100;
...
void KLineUtil::GetDrawTime(SHisQuoteData d, wchar_t* wtext, int size)
{
	...
	SDateType year = date / 10000;
	SDateType month = date / 100 % 100;
	SDateType day = date % 100;
	swprintf_s(wtext, size, L"%04d/%02d/%02d", year, month, day);
}
...
void KLineUtil::DrawTrend(TMemDC& dc, POINT from, POINT to)
{
	KLineSaveDC Save(dc);
	...
	MoveToEx(dc.GetHdc(), from.x, from.y, NULL);
	LineTo(dc.GetHdc(), to.x, to.y);

	double slopy = 0, cosy = 0, siny = 0;
	double par = 8.0; //length of Arrow (>)
	slopy = atan2(double(from.y - to.y), double(from.x - to.x));
	cosy = cos(slopy);
	siny = sin(slopy);
	//画箭头
	POINT pt1, pt2;
	pt1.x = to.x + int(par*cosy - (par / 2.0*siny));
	pt1.y = to.y + int(par*siny + (par / 2.0*cosy));
	pt2.x = to.x + int(par*cosy + par / 2.0*siny);
	pt2.y = to.y - int(par / 2.0*cosy - par * siny);
