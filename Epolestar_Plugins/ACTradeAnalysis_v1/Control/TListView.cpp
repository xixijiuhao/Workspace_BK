...
TListView::TListView()
{
	...
	m_hbr[1] = CreateSolidBrush(RGB(243, 243, 243));
	m_hbr[2] = CreateSolidBrush(RGB(209, 215, 229));
...
void TListView::CreateList(HWND hParent, UINT style, int itemHeight, int headerHeight, UINT exStyle, int index)
{
	...
	ListView_SetBkColor(m_hwndList, RGB(255, 255, 255));
}
...
void TListView::OnDrawItem(LPARAM lParam)
{
	...
	__int64 endtime = 0;
	__int64 begintime = GetCycleCount();
	...
	{
		item.iSubItem = i;
		SendMessage(m_hwndList, LVM_GETITEM, 0, (LPARAM)(LV_ITEM*)(&item));
		...
		if (lstrlen(szText))
		{
			...
			DrawText(pDIS->hDC, szText, -1, &rTemp, textFormat);
		}
	...
	double spent = (endtime - begintime)*1.0 / (double)fre.QuadPart;
}
...
void TListView::AddItem(const TCHAR *pszText, int iRow, int iCol, UINT text_format, COLORREF textColor)
{
	if (!pszText)return;
	if (!m_bVirtualMode)
	{
		...
		if (iCol == 0)
			SendMessage(m_hwndList, LVM_INSERTITEM, 0, (LPARAM)(&item));
		else
			SendMessage(m_hwndList, LVM_SETITEM, 0, (LPARAM)(&item));
	}
	else
	{
	...
	SetItemState(iRow, iCol, text_format, textColor);
}
void TListView::AddItemVM(const TCHAR *_pchar, int iRow, int iCols)
{
	if (iRow < m_vecRowData.size())
	{
		if (iCols < m_vecRowData[iRow].size())
			m_vecRowData[iRow][iCols] = _pchar;
	}
	else
	{
...
void TListView::AddItemVM(const char *_pchar, int iRow, int iCols)
{
	...
	{
		if (iCols < m_vecRowData[iRow].size())
			m_vecRowData[iRow][iCols] = ANSIToUnicode(_pchar);
	}
	else
	{
		int iColumns = Header_GetItemCount(m_hwndHeader);
		vector<wstring>vecOneRow(iColumns);
		if (iCols<iColumns)
			vecOneRow[iCols] = ANSIToUnicode(_pchar);
		m_vecRowData.push_back(vecOneRow);
	}
}
...
void TListView::SetItemState(int iRow, int iCol, UINT textFormat, COLORREF textColor)
{
	...
	{ //如果m_vItemState大于0，说明已经进行过插入，现在在第0行插入，其他行会依次往后面移动，所以这个的style要放在最前面
		vector<unsigned short>oneRow(m_iColCount);
		oneRow[0] = style;
...
void TListView::SelectItem(int index)
{
	...
	{
		int i = GetSelectItem();
		ListView_SetSelectionMark(m_hwndList, -1);
		ListView_SetItemState(m_hwndList, i, 0, LVIS_SELECTED | LVIS_FOCUSED);
...
void TListView::OnHeaderPaint()
{
	...
	SetTextColor(memdc.GetHdc(), RGB(30, 30, 30));
	SelectObject(memdc.GetHdc(), g_FontData.g_FontWord12);

	GetClientRect(m_hwndHeader, &rect);
	HBRUSH hbrBorder = CreateSolidBrush(RGB(240, 240, 240));
	FillRect(memdc.GetHdc(), &rect, hbrBorder);
	HPEN  hPen = CreatePen(PS_SOLID, 1, RGB(209, 215, 229));
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (i > 0)
		{
			MoveToEx(memdc.GetHdc(), rect.left, rect.top, 0);
			LineTo(memdc.GetHdc(), rect.left, rect.bottom);
		}
