...
public:
	TListView();
	TListView(const TListView &lv)
	{
		m_hwndList = lv.m_hwndList;
		m_hwndHeader = lv.m_hwndHeader;
		m_iHeaderHeight = lv.m_iHeaderHeight;
		m_iItemHeight = lv.m_iItemHeight;
		m_iColCount = lv.m_iColCount;
		m_hbr[0] = lv.m_hbr[0];
		m_hbr[1] = lv.m_hbr[1];
		m_hbr[2] = lv.m_hbr[2];
		m_vItemColor = lv.m_vItemColor; //必须有一个元素，默认item字体颜色
		m_vItemState = lv.m_vItemState;

		m_vecRowData = lv.m_vecRowData;
		m_bVirtualMode = lv.m_bVirtualMode;
