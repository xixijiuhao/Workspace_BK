...
void CPopWinList::CreatePopWin(IOperator *p, HWND parent,int nwidth, int nleft, int ntop, int nRef, wchar_t* sMem[10], int nSize, int nRow, int nCol, bool bCombox)
{
...
void CPopWinList::UpdateData(wchar_t* sMem[10], int nSize)
{
	m_vstring.clear();
...
LRESULT CPopWinList::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		{
			ShowWindow(m_Hwnd, SW_HIDE);
			m_pOper->SetContractNo( LoadRC::unicode_to_ansi(m_vstring[m_nPos].c_str()));
		}
		break;
	case WM_DESTROY:
		DeleteObject(m_hBKBitmap);
