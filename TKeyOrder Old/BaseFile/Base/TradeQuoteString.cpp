...
void CConvert::DefaultSendOrder(TSendOrder&SendOrder)
{
	...
	if (g_pGlobalModel->m_utUserType == utInner)
		SendOrder.Hedge = g_pGlobalModel->m_stOptionConfig.Hedge;
	else
		SendOrder.Hedge = hNone;
...
wstring CConvert::UTF8ToUnicode(const string& str, UINT nCode)
{
    size_t  len = 0;
    len = str.length();
    int  unicodeLen = ::MultiByteToWideChar(nCode, 0, str.c_str(), -1, nullptr, 0);

    wchar_t *  pUnicode;
    pUnicode = new  wchar_t[unicodeLen + 1];
    memset(pUnicode, 0, (unicodeLen + 1) * sizeof(wchar_t));

    ::MultiByteToWideChar(nCode, 0, str.c_str(), -1, (LPWSTR)pUnicode, unicodeLen);
...
void CConvert::GetPrice(const TContractSingleKey * stContract, TDirect direct, int PriceType, double& price)
{
	...
	if (g_pStarApi && g_pStarApi->GetSnapshot(qNo, &QuoteData))
	{
		if (direct == dBuy)
		{
			...
			case ptSuperPrice://超价 默认比对手价超一个价位
			{
				price = QuoteData.Data[S_FID_BESTASKPRICE].Price;
				if (price == 0)
					price = QuoteData.Data[S_FID_LIMITUPPRICE].Price;
...
TKUserType CConvert::GetUserType(const char* userNo)
{
	TDataContainer stUser;
	g_pTradeData->GetAllUser(stUser);

	for (int index = 0; index < stUser.size(); index++)
	{
		const TUserInfo*pUser = (const TUserInfo*)stUser.at(index);
		if (pUser->pLogin && pUser->pLogin->Logined == bYes)
		{
			if (strcmp(userNo, pUser->UserNo) == 0)
			{
				TKUserType ut = utInner;
...
bool CConvert::IsUserLogin()
{
	...
	{
		return IsLogined();
		//bool bLogin = IsLogined();//判断是否有用户登录
		//if (!bLogin)//如果没有
		//{
		...
		//	return IsLogined(); //再次判断用户是否登录成功
		//}
		//return true; //用户已经登陆，那么直接返回true
	}
	return false;
}