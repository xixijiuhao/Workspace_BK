...
{
	char strText[100] = "";
	if (ContractKey.CommodityType == ctFutures)//期货,现货、现货延期
	{
...
bool CConvert::GetQuotCommodityInf(const char*code, SOFCommodityInf & sCommodityInfo)
{
	...
	int num = g_StarApi->GetCommodityData("", SCommodityNo, &tmpCommodity, 1, false);
...
SPriceType CConvert::GetPriceByContract(const SContract* SContract, SFidMeanType nField)//（含隐含）
{
...
SQtyType CConvert::GetQtyByContract(const SContract* SContract, SFidMeanType nField)//（含隐含）
{
	if (!SContract || !g_StarApi)
		return 0;
