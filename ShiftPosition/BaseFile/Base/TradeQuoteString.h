...
namespace CConvert
{
	...
	//格式化交易合约结构体
	  ...
	  void GetPrice(TContractSingleKey *stContract, TDirect direct, int PriceType, double& price, int iSuperPot = 1);
	  void GetCloseNum(char* UserNo, TContractSingleKey* ContractKey, TDirect direct, const int iCoverQty, int& iCoverNum, int& iTCoverNum);
	  bool IsUserLogin();
