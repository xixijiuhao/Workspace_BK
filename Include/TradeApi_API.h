...
public:	
	...
	//风控注册 返回值0空指针或者已存在 1成功  nType=检测类型
	virtual int __cdecl RegistRiskModule(IRiskModuleApi * reg, int nType) = 0;

	//风控注销 返回值0空指针或者未找到 1成功
	virtual int __cdecl UnRegistRiskModule(IRiskModuleApi * reg) = 0;
};

class IRiskModuleApi
{
public:
	//检测撤单 返回0继续撤单 否则终止撤单操作 nType = 0(撤单检测,data类型TOrder)
	virtual int __cdecl CheckOrderOperation(const void *data,int nType) = 0;
};