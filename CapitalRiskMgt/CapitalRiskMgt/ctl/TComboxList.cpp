...
{
	m_font = g_FontData.GetFontWord12();
	m_nmouseover = -1;
	m_nleftalign = 10;
...
void TComboxList::OnPaint()
{
	...
	FrameRect(memdc.GetHdc(), &rect, g_ColorRefData.g_HbrGray);
...
void TComboxList::DrawMain(TMemDC* pmemdc, RECT& rect)
{
	...
	for (itor = m_stldata.begin(); itor != m_stldata.end(); itor++, nindex++)
	{
		...
		//sztext = szout;
		//sprintf_s(szout, "%-20s", szvalue.c_str());
		//sztext.append(szout);

		DrawTextA(pmemdc->GetHdc(), sztext.c_str(), -1, &rctemp, DT_LEFT | DT_VCENTER);
