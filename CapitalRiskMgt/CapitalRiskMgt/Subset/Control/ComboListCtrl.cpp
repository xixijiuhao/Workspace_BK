...
CInPlaceEditEx* CComboListCtrl::ShowInPlaceEditEx(int iRowIndex, int iColumnIndex, CString& rstrCurSelection)
{
	// Create an in-place edit control
	CInPlaceEditEx* pInPlaceEditEx = CInPlaceEditEx::GetInstance();
		
	CRect obCellRect(0, 0, 0, 0);
	CalculateCellRect(iColumnIndex, iRowIndex, obCellRect);
			
	pInPlaceEditEx->ShowEditCtrl(m_dwEditCtrlStyle, obCellRect, this, 0, 
							   iRowIndex, iColumnIndex,
							   m_strValidChars[iColumnIndex], rstrCurSelection);

	return pInPlaceEditEx;
}
	...
	if (-1 != iRowIndex)
	{
		...
		if ((GetItemState(iRowIndex, flag ) & flag) == flag)
		{
			...
			if (GetWindowLong(m_hWnd, GWL_STYLE) & LVS_EDITLABELS)
			{
				...
				{
					CInPlaceEditEx* pInPlaceEditEx = ShowInPlaceEditEx(iRowIndex, iColumnIndex, strCurSelection);
				}
			}
		}
	}  
...
bool CComboListCtrl::HitTestEx(CPoint &obPoint, int* pRowIndex, int* pColumnIndex) const
{
	...
	// Get the row index
	*pRowIndex = HitTest(obPoint, nullptr);
