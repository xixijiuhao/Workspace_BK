...
void VirtualWnd::OnPaint()
{
	...
	rc.right = Wnd_Width; 
	rc.top = rect.top;  
	rc.bottom = rect.top + CAPTION_HEIGHT;
	...
	//关闭小×
	m_rectMini.left = m_rectClose.left + 10; 
	m_rectMini.right = m_rectClose.right - 10;
	m_rectMini.top = m_rectClose.top + 10; 
	m_rectMini.bottom = m_rectClose.bottom - 10;
	//画关闭按钮
	if (m_bhoverclose)
	{
		rc = m_rectClose; 
		rc.bottom = m_rectClose.bottom;
