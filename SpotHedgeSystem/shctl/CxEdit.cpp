...
LRESULT CALLBACK CxEdit::WndProc(UINT message, WPARAM w, LPARAM l)
{
	...
	else if (WM_KEYDOWN == message )
	{
		...
		if (w == VK_UP)
		{
			SendMessage(GetParent(m_hWnd), SSWM_EDIT_PRESS_UP, GetWindowLongPtr(m_hWnd, GWL_ID), LPARAM(m_hWnd));
			return 0;
		}
		if (w == VK_DOWN)
		{
			SendMessage(GetParent(m_hWnd), SSWM_EDIT_PRESS_DOWN, GetWindowLongPtr(m_hWnd, GWL_ID), LPARAM(m_hWnd));
			return 0;
		}
		if (w == VK_RIGHT)
		{
			SendMessage(GetParent(m_hWnd), SSWM_EDIT_PRESS_RIGHT, GetWindowLongPtr(m_hWnd, GWL_ID), LPARAM(m_hWnd));
		}
	}
	else if (WM_LBUTTONDBLCLK == message)
	{
		SendMessage(GetParent(m_hWnd), SSWM_EDIT_DBLCLK, GetWindowLongPtr(m_hWnd, GWL_ID), LPARAM(m_hWnd));
		return 0;
	}
...
void CxEdit::Create(HWND hParent, UINT style,UINT specialStyle, UINT id)
{
	...
	HWND hedit = CreateWindowEx(0, WC_EDIT, TEXT(""), ES_AUTOVSCROLL | WS_CHILD | WS_VISIBLE | WS_BORDER | WS_TABSTOP | WS_CLIPSIBLINGS |specialStyle,
...
{
	char szText[100] = { 0 };
	sprintf_s(szText, "%d", number);
	SetWindowTextA(m_hWnd, szText);
}
void CxEdit::SetDoubleData(double data, int iPresicion)
{
	char szText[100] = { 0 };
	char format[20] = { 0 };
