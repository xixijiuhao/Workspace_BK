...
TAgreementPopWnd::TAgreementPopWnd()
{
...
bool TAgreementPopWnd::Create(HWND hparent)
{
	m_parent = hparent;
...
void TAgreementPopWnd::SetData(const std::vector<string>& stldata)
{
	m_stldata = stldata;
	m_nmouseover = -1;
}

void TAgreementPopWnd::GetData(char * sztext, string & stdata)
{
...
int TAgreementPopWnd::GetListHeight()
{
	int nheight;
...
void TAgreementPopWnd::MoveWindow(const int & x, const int & y, const int & cx, const int & cy)
{
	int nwidth, nheight;
	nheight = GetLineHeight() * m_stldata.size();
	nheight = min(nheight, m_nmaxheight);
...
LRESULT TAgreementPopWnd::WndProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_PAINT:
...
void TAgreementPopWnd::DrawMain(TMemDC * pmemdc, RECT & rect)
{
	...
	for (itor = m_stldata.begin(); itor != m_stldata.end(); itor++, nindex++)
	{
		...
		sprintf_s(szout, "%-10s", sztext.c_str());
		sztext = szout;
...
void TAgreementPopWnd::OnSize()
{
	ResetScrollBars();
}

void TAgreementPopWnd::OnLbuttonDown(WPARAM wParam, LPARAM lParam)
{
	RECT rect;
	GetClientRect(m_Hwnd, &rect);
...
void TAgreementPopWnd::OnMouseMove(WPARAM wParam, LPARAM lParam)
{
	RECT rect;
	GetClientRect(m_Hwnd, &rect);
...
void TAgreementPopWnd::OnVScroll(WPARAM wParam, LPARAM lParam)
{
...
void TAgreementPopWnd::OnMouseWheel(WPARAM wParam, LPARAM lParam)
{
...
void TAgreementPopWnd::OnMouseLeave()
{
	m_btrack = false;
	m_nmouseover = -1;
...
int TAgreementPopWnd::GetLineHeight()
{
	return 25;
}

int TAgreementPopWnd::GetLineWidth()
{
	HDC hdc;
	hdc = GetDC(NULL);
	std::vector<string>::iterator itor;
...
void TAgreementPopWnd::ResetScrollBars(bool brestpos)
{
...
void TAgreementPopWnd::RefreshData()
{
	InvalidateRect(m_Hwnd, 0, false);
}

bool TAgreementPopWnd::IsOnlyShow()
{
	int nindex = 0;
	std::vector<string>::iterator itor;
	for (itor = m_stldata.begin(); itor != m_stldata.end(); itor++)
