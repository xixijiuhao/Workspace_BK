...
std::string NewClientName(const char* srvtype, const char* mac)
{
	char buf[128];
	CDate now;
	sprintf_s(buf,"%s%s%d",srvtype,mac,now.GetTimeStamp() ^ now.GetDate() ^ OperateSystem::getCurrentProcessId());
	return buf;
}
...
bool GetIpAddressByUrl(char* ip, const char* inurl)
{
	...
	{//tcp:\\... or http:\\...
		...
		{
			strncpy_s(name, sizeof(name), url, end - url);
		}
	}
	else
	{
		strncpy_s(name, sizeof(name), inurl, url - inurl);
	}
...
bool GetLocalMACIP(char* macAddress, char* Ip,const char* desturl)
{
	...
	{
		strcpy_s(macAddress, sizeof(macAddress),"Unknown");
	...
	if(NULL != pAdapterInfo)
	{
		...
		if ((dwRet = GetAdaptersInfo(pAdapterInfo, &ulBufLen)) == NO_ERROR) 
		{
			...
			if(NULL != pAdapter)
			{
				...
				{
					//无格式
					sprintf_s(macAddress + i * 2, sizeof(macAddress) - i * 2, "%02X", (int)pAdapter->Address[i]);
				}
				sprintf_s(Ip,sizeof(Ip),"%s", pAdapter->IpAddressList.IpAddress.String);
				bfind = true;
			}
			else
			{
				sprintf_s(macAddress,sizeof(macAddress),"Unknown");
				bfind = false;
			}
			FREE(pAdapterInfo);
		}
...
char* GetDateByString()
{
	...
	tm * pTmBlock = nullptr;
	localtime_s(pTmBlock,&timer);
	sprintf_s(szDate, "%04d%02d%02d", pTmBlock->tm_year + 1900, pTmBlock->tm_mon + 1, pTmBlock->tm_mday);
	return *(&szDate);
}
char szTime[9];
char* GetTimeByString()
{
	...
	tm * pTmBlock = nullptr;
	localtime_s(pTmBlock, &timer);
	sprintf_s(szTime, "%02d:%02d:%02d", pTmBlock->tm_hour, pTmBlock->tm_min, pTmBlock->tm_sec);
	return szTime;
}
...
timeval CurrentTimeTag()
{
	...
	timeval tv;
	tv.tv_sec = (long)ln64Sec;
	tv.tv_usec = (long)ln64uSec;
	return tv;
#endif
#ifdef LINUX
	struct timeval tv;
...
{	
	sprintf_s(time, sizeof(time), "%02d%c%02d%c%02d", m_ihour, splitchar, m_iMinutes, splitchar, m_iSeconds);
}
...
void OperateSystem::getComputerName(char* compName, int buflen)
{
	...
	{
		strncpy_s(compName,sizeof(compName),"UnknowCompName",-1);
	}
#endif
#ifdef LINUX
...
void OperateSystem::getUserName(char* username, int buflen)
{
	...
	{
		strncpy_s(username,sizeof(username),"UnknowUserName",dbuflen-1);
	}
#endif
#ifdef LINUX
